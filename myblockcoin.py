...
class TCPHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        # Ajout pour comprendre de quoi il s'agit:
        if command == "block":
            block = bank.handle_block(data)
            self.respond(command="block-response", data=block)
...
def main(args):
    if args["serve"]:
        ...
        response = send_message(('node0',PORT), "balance", public_key, response=True)
        print(response)
    elif args["tx"]:
        # Grab parameters
